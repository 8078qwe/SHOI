...
class HOITransHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.query_embed_h = nn.Embedding(self.num_query, self.embed_dims)
    ...
    def forward(self, feats, img_metas):
        ...
        h_hs, o_hs, inter_hs, memory = self.transformer(last_features, masks,
                                            self.query_embed_h.weight,
                                            self.query_embed_o.weight,
                                            self.pos_guided_embedd.weight,
                                            pos_embed)

        sub_outputs_class = self.sub_cls_embed(h_hs)
        sub_outputs_coord = self.sub_box_embed(h_hs).sigmoid()
        obj_outputs_class = self.obj_cls_embed(o_hs)
        obj_outputs_coord = self.obj_box_embed(o_hs).sigmoid()

        all_cls_scores = dict(sub=sub_outputs_class, obj=obj_outputs_class)
        rel_outputs_class = self.rel_cls_embed(inter_hs)
        all_cls_scores['rel'] = rel_outputs_class
        if self.use_mask:
            ###########for segmentation#################
            sub_bbox_mask = self.sub_bbox_attention(h_hs[-1],
                                                    memory,
                                                    mask=masks)
            obj_bbox_mask = self.obj_bbox_attention(o_hs[-1],
                                                    memory,
                                                    mask=masks)
